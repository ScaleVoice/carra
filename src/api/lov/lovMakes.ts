...
const fetchMakes = async (searchParams?: QueryParams): Promise<LovMakesResponse> => {
...
export function useMakeList(
  ...
  const { data, ...rest } = useLovMakesQuery(
    { active: true },
    { enabled, queryKey: QUERY_KEYS.lovMakes({ active: true }) },
  )
  ...
  const popularMakes = useMemo(
    () =>
      carMakes
        ...
        .sort((a, b) => (a.popularity && b.popularity ? a.popularity - b.popularity : 0))
  ...
  const filteredCarMakes = useMemo(() => {
    ...
    return carMakes.filter((make) => make.name?.toLowerCase().includes(search.toLowerCase()))
  }, [carMakes, search])
...
export function sortMakes(a: { value: string; label: string }, b: { value: string; label: string }) {
  if (!a.label || !b.label) {
    return 0
  }
