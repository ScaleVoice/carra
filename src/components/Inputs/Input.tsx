...
export type InputProps = Omit<React.HTMLProps<HTMLInputElement>, "onChange" | "onFocus" | "size" | "ref"> &
  InputVariantProps & {
...
const sideVariants = cva(
  ["flex items-center self-stretch border-0 justify-center", "peer-focus:border-gray-300 group-hover:border-gray"],
  {
    variants: {
      orientation: {
        right: ["border-l"],
...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ renderLeft, renderRight, name, size, disabled, error, className, containerClassName, ...rest }, ref) => {
    ...
    return (
      ...
      >
        {renderLeft && <>{renderLeft(sideVariants({ orientation: "left", size }), error)}</>}
        ...
        {renderRight && <>{renderRight(sideVariants({ orientation: "right", size }), error)}</>}
      </div>
    )
  },
)
